<?php
namespace MuseaCore\CPT\Shortcodes\SocialShare;
...
class SocialShare implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Social Share', 'musea-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'value'      => array(
								esc_html__( 'List', 'musea-core' )     => 'list',
								esc_html__( 'Dropdown', 'musea-core' ) => 'dropdown',
								esc_html__( 'Text', 'musea-core' )     => 'text'
							)
						),
						array(
							'type'       => 'dropdown',
							'param_name' => 'dropdown_behavior',
							'heading'    => esc_html__( 'DropDown Hover Behavior', 'musea-core' ),
							'value'      => array(
								esc_html__( 'On Bottom Animation', 'musea-core' ) => 'bottom',
								esc_html__( 'On Right Animation', 'musea-core' )  => 'right',
								esc_html__( 'On Left Animation', 'musea-core' )   => 'left'
							),
						...
						array(
							...
							'heading'    => esc_html__( 'Icons Type', 'musea-core' ),
							'value'      => array(
								esc_html__( 'Font Awesome', 'musea-core' ) => 'font-awesome',
								esc_html__( 'Font Elegant', 'musea-core' ) => 'font-elegant'
							),
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'title',
							'heading'    => esc_html__( 'Social Share Title', 'musea-core' )
						)
					)
				)
			);
	...
	public function render( $atts, $content = null ) {
		...
		$params['enable_social_share'] = musea_elated_options()->getOptionValue( 'enable_social_share' ) === 'yes';
		
		//Is social share enabled for post type
		$post_type         = str_replace( '-', '_', get_post_type() );
		$params['enabled'] = musea_elated_options()->getOptionValue( 'enable_social_share_on_' . $post_type ) === 'yes';
		...
		if ( $params['enable_social_share'] && $params['enabled'] ) {
			$html = musea_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'social-share', '', $params );
		}
	...
	private function getSocialNetworksParams( $params ) {
		...
		foreach ( $this->socialNetworks as $net ) {
			...
			if ( musea_elated_options()->getOptionValue( 'enable_' . $net . '_share' ) == 'yes' ) {
				...
				$params['custom_icon'] = ( musea_elated_options()->getOptionValue( $net . '_icon' ) ) ? musea_elated_options()->getOptionValue( $net . '_icon' ) : '';
				
				$html = musea_core_get_shortcode_module_template_part( 'templates/parts/network', 'social-share', '', $params );
			}
    ...
    private function getSocialNetworkShareLink($net, $image) {
        switch ($net) {
            ...
            case 'twitter':
                ...
                $twitter_via = (musea_elated_options()->getOptionValue('twitter_via') !== '') ? esc_attr__( ' via ', 'musea-core' ) . musea_elated_options()->getOptionValue('twitter_via') . ' ' : '';
	            $link =  'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode( musea_elated_the_excerpt_max_charlength( $count_char ) . $twitter_via ) . ' ' . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                break;
            case 'linkedin':
	...
	private function getSocialNetworkText( $net ) {
		switch ( $net ) {
			case 'facebook':
				$text = esc_html__( 'Facebook', 'musea-core');
				break;
			case 'twitter':
				$text = esc_html__( 'Twitter', 'musea-core');
				break;
			case 'linkedin':
				$text = esc_html__( 'LinkedIN', 'musea-core');
				break;
			case 'tumblr':
				$text = esc_html__( 'Tumblr', 'musea-core');
				break;
			case 'pinterest':
				$text = esc_html__( 'Pinterest', 'musea-core');
				break;
			case 'vk':
				$text = esc_html__( 'vk', 'musea-core');
				break;
			default:
				$text = '';
		}
