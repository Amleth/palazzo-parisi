<?php
namespace MuseaCore\CPT\Shortcodes\Icon;
...
class Icon implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
	    if(function_exists('vc_map')) {
		    vc_map(
		    	array(
				    'name'                      => esc_html__( 'Icon', 'musea-core' ),
				    'base'                      => $this->base,
				    ...
				    'params'                    => array_merge(
					    ...
					    array(
						    array(
							    ...
							    'value'      => array(
								    esc_html__( 'Tiny', 'musea-core' )   => 'eltdf-icon-tiny',
								    esc_html__( 'Small', 'musea-core' )  => 'eltdf-icon-small',
								    esc_html__( 'Medium', 'musea-core' ) => 'eltdf-icon-medium',
								    esc_html__( 'Large', 'musea-core' )  => 'eltdf-icon-large',
								    esc_html__( 'Huge', 'musea-core' )   => 'eltdf-icon-huge'
							    )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'custom_size',
							    'heading'    => esc_html__( 'Custom Size (px)', 'musea-core' )
						    ),
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'type',
							    ...
							    'value'      => array(
								    esc_html__( 'Normal', 'musea-core' ) => 'eltdf-normal',
								    esc_html__( 'Circle', 'musea-core' ) => 'eltdf-circle',
								    esc_html__( 'Square', 'musea-core' ) => 'eltdf-square'
							    ),
							    'save_always' => true
						    ),
						    array(
							    ...
							    'heading'     => esc_html__( 'Border Radius', 'musea-core' ),
							    'description' => esc_html__( 'Please insert border radius(Rounded corners) in px. For example: 4 ', 'musea-core' ),
							    'dependency'  => array( 'element' => 'type', 'value' => array( 'eltdf-square' ) )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'shape_size',
							    'heading'    => esc_html__( 'Shape Size (px)', 'musea-core' ),
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Border Width', 'musea-core' ),
							    'dependency' => array( 'element' => 'type', 'value'   => array( 'eltdf-circle', 'eltdf-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Hover Icon Color', 'musea-core' )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Icon Animation', 'musea-core' ),
							    'value'      => array_flip( musea_elated_get_yes_no_select_array( false ) )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'icon_animation_delay',
							    'heading'    => esc_html__( 'Icon Animation Delay (ms)', 'musea-core' ),
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Use Link as Anchor', 'musea-core' ),
							    'value'       => array( 'Use this icon as Anchor?' => 'yes' ),
							    'description' => esc_html__( 'Check this box to use icon as anchor link (eg. #contact)', 'musea-core' ),
							    'dependency'  => Array( 'element' => 'link', 'not_empty' => true )
						    ),
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'target',
							    'heading'    => esc_html__( 'Target', 'musea-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$html = musea_core_get_shortcode_module_template_part( 'templates/icon', 'icon', '', $params );
	...
	private function generateIconStyles( $params ) {
		...
		if ( ( $params['type'] !== 'eltdf-normal' && ! empty( $params['shape_size'] ) ) || ( $params['type'] == 'eltdf-normal' ) ) {
			if ( ! empty( $params['custom_size'] ) ) {
				$iconStyles[] = 'font-size:' . musea_elated_filter_px( $params['custom_size'] ) . 'px';
			}
		}
	...
	private function generateIconHolderStyles( $params ) {
		...
		if ( $params['type'] !== 'eltdf-normal' ) {
			...
			if ( ! empty( $shapeSize ) ) {
				$iconHolderStyles[] = 'width: ' . musea_elated_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'height: ' . musea_elated_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'line-height: ' . musea_elated_filter_px( $shapeSize ) . 'px';
			}
			...
			if ( ! empty( $params['border_color'] ) && ( isset( $params['border_width'] ) && $params['border_width'] !== '' ) ) {
				...
				$iconHolderStyles[] = 'border-width: ' . musea_elated_filter_px( $params['border_width'] ) . 'px';
			...
			if ( $params['type'] == 'eltdf-square' ) {
				if ( isset( $params['border_radius'] ) && $params['border_radius'] !== '' ) {
					$iconHolderStyles[] = 'border-radius: ' . musea_elated_filter_px( $params['border_radius'] ) . 'px';
				}
			}
		}
